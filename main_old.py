...
class Problem:
        ...
        for cache in range(self.density_matrix.shape[0]):
            for video in range(self.density_matrix.shape[1]):
                if (cache, video) in self.requests_by_cache_video:
                    self.density_matrix[cache,video] = np.random.random()

        try:
            print('ya puedes interrumpir')
            n_iter = 0
            while True:
                n_iter += 1
                num_no_zero = np.sum(self.density_matrix != 0.0)
                if num_no_zero == 0:
                    break
    ...
    def print_output(self):
        cs = {}
        with open('kittens.out', 'w') as f:
            for i, c in enumerate(self.caches):
                if len(c['videos']) != 0:
                    cs[i] = c['videos']
            f.write(str(len(cs))+'\n')
            for i, c in cs.items():
                f.write(" ".join(str(a) for a in ([i] + list(c))) + '\n')
